...
// POST - Verify payment status from Zwitch and update (for external payment apps)
router.post("/:id/verify-payment-status", async (req, res) => {
  try {
    const { id } = req.params;
    const { paymentToken } = req.body;

    console.log("Verify payment status request:", { id, paymentToken });
    ...
    if (selection.paymentStatus === "completed" && selection.paymentMode === "online") {
      console.log("Payment already completed for:", id);
      return res.json({
        success: true,
    ...
    // Get Zwitch configuration
    const ZWITCH_API_KEY = process.env.ZWITCH_PG_API_KEY || "a55ad409-80be-4d64-b3bf-4df213e74c18";
    const ZWITCH_API_SECRET = process.env.ZWITCH_PG_API_SECRET || "sk_test_NtRxMXMvoCjQBmYKqVSaY3zWBLgDplE9";
    const ZWITCH_API_URL = process.env.ZWITCH_PG_API_URL || "https://api.zwitch.io";

    // Call Zwitch API to check payment status
    const bearerToken = `${ZWITCH_API_KEY}:${ZWITCH_API_SECRET}`;
    const statusUrl = `${ZWITCH_API_URL}/v1/pg/payment_token/${paymentToken}`;

    console.log("Calling Zwitch status API:", statusUrl);
    ...
    try {
      ...
      console.log("Zwitch status response:", response.data);
      ...
      if (paymentStatus === "captured" || paymentStatus === "success" || paymentStatus === "paid") {
        console.log("Payment verified as successful:", { paymentId, amount, paymentStatus });

        // Update the selection with payment details
        ...
        const previousAmount = selection.paidAmount || 0;
        selection.paidAmount = previousAmount + amount;
        ...
        selection.driverPayments.push({
          ...
          amount: amount,
          mode: "online",
          type: "security", // Default to security deposit
          status: "captured",
          paymentId: paymentId,
        ...
        selection.depositPaid = (selection.depositPaid || 0) + amount;
        ...
        try {
          ...
          if (selection.driverSignupId) {
            try {
              ...
              console.warn(`Notification lookup failed: ${lookupErr.message}`);
            }
          }
          ...
          if (recipientId) {
            await createAndEmitNotification({
              type: "driver_payment",
              title: `Payment Successful: ₹${amount.toLocaleString("en-IN")}`,
              message: `Your payment of ₹${amount.toLocaleString("en-IN")} has been confirmed.`,
              data: {
                selectionId: selection._id,
                paymentId: paymentId,
                amount: amount,
        ...
        console.log("Payment not completed yet:", { paymentStatus, paymentId });
        return res.json({
          success: true,
      ...
      console.error("Error calling Zwitch API:", {
        status: zwitchError.response?.status,
        data: zwitchError.response?.data,
        message: zwitchError.message,
      });

      // If Zwitch API fails, return current status
      return res.json({
        success: false,
        message: "Could not verify payment status from gateway",
        error: zwitchError.response?.data || zwitchError.message,
        selection: selection,
      });
    }
  } catch (error) {
